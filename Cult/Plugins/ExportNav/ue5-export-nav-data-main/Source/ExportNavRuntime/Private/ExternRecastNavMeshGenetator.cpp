...
void FExternExportNavMeshGenerator::GrowConvexHull(const dtReal ExpandBy, const TArray<FVector>& Verts, TArray<FVector>& OutResult)
{
	...
	struct FSimpleLine
	{
		...
		{
			const dtReal A1 = Line1.P2.X - Line1.P1.X;
			const dtReal B1 = Line2.P1.X - Line2.P2.X;
			const dtReal C1 = Line2.P1.X - Line1.P1.X;

			const dtReal A2 = Line1.P2.Y - Line1.P1.Y;
			const dtReal B2 = Line2.P1.Y - Line2.P2.Y;
			const dtReal C2 = Line2.P1.Y - Line1.P1.Y;

			const dtReal Denominator = A2 * B1 - A1 * B2;
			if (Denominator != 0)
			{
				const dtReal t = (B1*C2 - B2 * C1) / Denominator;
	...
	for (int32 Index = 0; Index < VertsCount - 2; ++Index)
	{
		...
		const dtReal d = FVector::DotProduct(NV1, V12);
	...
	const dtReal ExpansionThreshold = 2 * ExpandBy;
	const dtReal ExpansionThresholdSQ = ExpansionThreshold * ExpansionThreshold;
	...
	for (int32 Index = 0; Index < VertsCount - 2; ++Index)
	{
		...
		else
		{
			...
			const dtReal DistToNewVector = VectorToNewPoint.SizeSquared2D();
